#!/usr/bin/env python3
"""Generate canonical empirical outputs under the author-approved independence-GEE policy.
...
covariance. This script does not overwrite any prior/conflicting output.
"""
...
def main() -> int:
        ...
        for model_id, dat, formula, subset in specs:
            ...
            demo_rows.extend(
                model_rows(
                    result=res,
                    data=dat,
                    output_name="primary_demographic_sensitivity_CURRENT.csv",
    ...
    write_csv(demo_rows, out / "primary_demographic_sensitivity_CURRENT.csv")
    ...
    strict_rows = model_rows(
        result=strict_res,
        data=strict,
        output_name="strict_acoustic_50trial_sensitivity_CURRENT.csv",
    ...
    write_csv(strict_rows, out / "strict_acoustic_50trial_sensitivity_CURRENT.csv")
    ...
    for dat, input_path, outcome, mechanistic_gate in interaction_specs:
        ...
        interaction_rows.extend(
            model_rows(
                result=res,
                data=dat,
                output_name="interaction_models_summary_CURRENT.csv",
    ...
    write_csv(interaction_rows, out / "interaction_models_summary_CURRENT.csv")
    write_csv(slope_rows, out / "interaction_slopes_by_rate_CURRENT.csv")
    ...
    for model_id_base, col_a, col_b, phase_a, phase_b in phase_specs:
        ...
        rows = model_rows(
            result=res,
            data=long,
            output_name="phase_interaction_models_CURRENT.csv",
    ...
    write_csv(phase_rows, out / "phase_interaction_models_CURRENT.csv")
        ...
        for model_short, outcome, formula, is_log in specs:
            ...
            coupling_rows.extend(
                model_rows(
                    result=res,
                    data=dat,
                    output_name="coupling_trial_level_models_CURRENT.csv",
    ...
    write_csv(coupling_rows, out / "coupling_trial_level_models_CURRENT.csv")
    ...
    read_rows = model_rows(
        result=read_res,
        data=read,
        output_name="read_speech_effects_CURRENT.csv",
    ...
    write_csv(read_rows, out / "read_speech_effects_CURRENT.csv")

    # Run log and output hashes.
    output_files = [
        "primary_demographic_sensitivity_CURRENT.csv",
        "strict_acoustic_50trial_sensitivity_CURRENT.csv",
        "interaction_models_summary_CURRENT.csv",
        "interaction_slopes_by_rate_CURRENT.csv",
        "phase_interaction_models_CURRENT.csv",
        "coupling_trial_level_models_CURRENT.csv",
        "read_speech_effects_CURRENT.csv",
    ]
    ...
    log_lines += [
        ...
        "The CSV files record full-precision values produced in this environment. Comparisons to prior outputs should allow only floating-point rounding at approximately 1e-10 absolute for coefficients and standard errors when the model, input, and software version are otherwise identical. Results generated under a different working correlation or model class are not numerical-tolerance differences and are retained as prior/conflicting specifications.",
    ]
    (out / "CANONICAL_EMPIRICAL_MODELS_CURRENT_RUNLOG.md").write_text("\n".join(log_lines) + "\n", encoding="utf-8")
