...
This runner executes the analysis families retained in the manuscript: demographic/female sensitivity, strict
...
def main():
    ...
    canon_path=root/'analysis/empirical/model_functions.py'; canon=load_canonical(canon_path)
    ...
    for outcome in ['log_speechrate','log_articulationrate']:
      ...
      for model_id,dat,formula,subset in specs:
        ...
        rows += canon.model_rows(result=res,data=dat,output_name='demographic_sensitivity.csv',analysis_family='primary_and_demographic_sensitivity',model_id=model_id,outcome=outcome,formula=formula,input_files=[paths['primary']],original_root=root,script_path=canon_path,status='reproduced',log_outcome=True,sequence_reference='bibibi',subset=subset)
    write(rows,out/'demographic_sensitivity.csv')
    # Strict acoustic.
    ...
    rows=canon.model_rows(result=res,data=strict,output_name='strict_acoustic_sensitivity.csv',analysis_family='strict_acoustic_50trial_sensitivity',model_id='audio_modulation_minimum_50_valid_trials_independence_gee',outcome='audio_mod_dom_hz',formula=formula,input_files=[paths['secondary']],original_root=root,script_path=canon_path,status='reproduced',log_outcome=False,sequence_reference='bibibi',subset=f'{len(eligible)} speakers with >=50 nonmissing audio_mod_dom_hz values',selection_rule='participant retained when count(nonmissing audio_mod_dom_hz) >= 50; model complete cases thereafter',extra={'eligible_speakers':';'.join(eligible)})
    write(rows,out/'strict_acoustic_sensitivity.csv')
    ...
    for dat,ip,outcome,gated in specs:
      ...
      rows += canon.model_rows(result=res,data=dat,output_name='interaction_models.csv',analysis_family='co_me_by_rate_interaction',model_id=f'{outcome}_by_rate_independence_gee',outcome=outcome,formula=formula,input_files=[ip],original_root=root,script_path=canon_path,status='reproduced',log_outcome=outcome.startswith('log_'),sequence_reference='bibibi',subset='analysis_ok and ema_cycles_ok' if gated else 'all primary timing observations',extra={'interaction_type':'Co--Me x instructed rate'})
    ...
    write(rows,out/'interaction_models.csv'); write(slopes,out/'interaction_slopes_by_rate.csv')
    # Read speech.
    ...
    rows=canon.model_rows(result=res,data=read,output_name='read_speech_effects.csv',analysis_family='read_speech',model_id='read_speech_independence_gee',outcome='audio_mod_dom_hz',formula=formula,input_files=[paths['read'],paths['anatomy']],original_root=root,script_path=canon_path,status='reproduced',log_outcome=False,sequence_reference='not applicable',rate_reference='not applicable',subset="status == ok and complete anthropometry",selection_rule="read-speech rows with status 'ok'; inner merge to finalized Co--Me and height",extra={'task_reference':'passage_baseline_noEMA'})
    write(rows,out/'read_speech_effects.csv')
    ...
    pd.DataFrame(availability).to_csv(out/'availability_models.csv',index=False,float_format='%.17g')
    checks=[]
    for name in ['demographic_sensitivity.csv','strict_acoustic_sensitivity.csv','interaction_models.csv','interaction_slopes_by_rate.csv','read_speech_effects.csv']:
      checks.append(compare(out/name,root/'results/canonical_empirical'/name))
    pd.DataFrame(checks).to_csv(out/'numerical_validation.csv',index=False)
