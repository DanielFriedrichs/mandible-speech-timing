#!/usr/bin/env python3
"""Generate publication figures, Table 1, and reproducible figure data products.

The script implements the specified participant-clustered Gaussian
...
required and set even though the figure design uses no random jitter.
"""
...
PRIMARY_SHA256 = "4492e389789b7126c2b9f15588b682871f374cf917aa6006134c3090bffcf9ed"
SECONDARY_SHA256 = "10c3eedaf2f91990cc128639cf0475eb1c1583c821171d99a1a26d7dddb7b6e0"
        ...
        p_out.append(
            {
                ...
                "authority": "comparison.csv; primary data hash verified",
            }
        )
...
def figure1(
    ...
    fig = plt.figure(figsize=(7.55, 3.25))
    gs = fig.add_gridspec(1, 3, width_ratios=[1.04, 1.04, 1.00], left=0.075, right=0.985, bottom=0.18, top=0.84, wspace=0.48)
    ...
    panel_label(ax, "C")
    xmin = min(-14.0, float(c["ci_lower"].min()) - 2.0)
    xmax = max(2.0, float(c["ci_upper"].max()) + 0.8)
    ax.set_xlim(xmin, xmax)
    ax.set_ylim(-0.45, 1.45)
    label_x = xmin + 0.15
    ax.text(label_x, 1.0, "Speech rate", ha="left", va="center", fontsize=7.1)
    ax.text(label_x, 0.0, "Articulation rate", ha="left", va="center", fontsize=7.1)
    ...
    pdf_meta = {"Title": "Figure 1 - mandibular length and rapid syllable timing", "Author": "Daniel Friedrichs and Volker Dellwo", "Creator": "Matplotlib", "CreationDate": None, "ModDate": None}
    png_meta = {"Title": "Figure 1 - mandibular length and rapid syllable timing", "Author": "Daniel Friedrichs and Volker Dellwo", "Software": "Matplotlib"}
...
def figure2(
    ...
    pdf_meta = {"Title": "Figure 2 - secondary kinematic and acoustic associations", "Author": "Daniel Friedrichs and Volker Dellwo", "Creator": "Matplotlib", "CreationDate": None, "ModDate": None}
    png_meta = {"Title": "Figure 2 - secondary kinematic and acoustic associations", "Author": "Daniel Friedrichs and Volker Dellwo", "Software": "Matplotlib"}
    fig.savefig(out_pdf, bbox_inches="tight", metadata=pdf_meta)
    fig.savefig(out_png, dpi=300, bbox_inches="tight", metadata=png_meta)
    plt.close(fig)
...
\footnotesize\textit{Note.} $N_{\mathrm{spk}}$ is the number of independent anatomical units (speakers); $N_{\mathrm{obs}}$ is the number of repeated within-speaker observations included in the outcome-specific model. Primary DDK rate effects are percent changes from log-outcome models; secondary effects are in the stated outcome units. The displayed intervals and \(P\) values use normal-reference robust sandwich inference. Small-cluster CR0/\(t_{G-1}\), CR2/Satterthwaite, participant-cluster bootstrap, and restricted-null wild-cluster bootstrap-\(t\) checks were performed only for the two primary outcomes and are reported in table~\ref{tab:supp_small_sample_robustness}.
\end{minipage}
\end{table}
"""
...
def main() -> int:
    ...
    s1.to_csv(out / "figure1_speaker_summaries.csv", index=False, float_format="%.17g")
    s2.to_csv(out / "figure2_speaker_summaries.csv", index=False, float_format="%.17g")
    predictions.to_csv(out / "figure_model_predictions.csv", index=False, float_format="%.17g")
    effects.to_csv(out / "figure_effect_estimates.csv", index=False, float_format="%.17g")
    ...
    write_table1(effects, out / "Table1.tex")
