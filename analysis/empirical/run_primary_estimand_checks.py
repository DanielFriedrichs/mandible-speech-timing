...
It produces:
  * PRIMARY_ESTIMAND_COMPARISON_CURRENT.csv
  * PRIMARY_ESTIMAND_ALL_COEFFICIENTS_CURRENT.csv
  * PRIMARY_ESTIMAND_RUNLOG_CURRENT.md
  * PRIMARY_ESTIMAND_DECISION_SUMMARY.md
...
CANONICAL_DATA_SHA256 = "25aaddf2ac0894b19bab058afe1fbe153b5e81b3ed4f744813e4e03c72094084"
...
def find_canonical_dataset(project_root: Path, explicit_data: Path | None) -> tuple[Path, list[Path]]:
    ...
    preferred_fragments = [
        "_github_staging_mandible_speech_timing/data/derived/analysis_dataset_clean.csv",
        "submission_clean_20260420_113327/reproducibility/key_results/outputs_paper_v1/analysis_dataset_clean.csv",
    ]
    for fragment in preferred_fragments:
        for p in candidates:
            if str(p).replace("\\", "/").endswith(fragment):
                return p, candidates
...
def write_decision_summary(
    ...
    lines.append(
        ...
        "author-approved participant-clustered independence-GEE specification with robust "
        "sandwich covariance."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append("## Author decision to record")
    lines.append("")
    lines.append(
        "Choose one primary construct before rewriting: (a) absolute external mandibular length, "
        "or (b) external mandibular length conditional on stature. The title, abstract, Introduction, "
        "Results, and Discussion must use the same construct."
    )
...
def main() -> int:
    ...
    all_path = output_dir / "PRIMARY_ESTIMAND_ALL_COEFFICIENTS_CURRENT.csv"
    ...
    compact_path = output_dir / "PRIMARY_ESTIMAND_COMPARISON_CURRENT.csv"
    compact.to_csv(compact_path, index=False, float_format="%.17g")

    summary_path = output_dir / "PRIMARY_ESTIMAND_DECISION_SUMMARY.md"
    write_decision_summary(compact, summary_path, chosen_data, co_mean, co_sd, co_range, shared_sequences)

    ended = datetime.now(timezone.utc)
    runlog_path = output_dir / "PRIMARY_ESTIMAND_RUNLOG_CURRENT.md"
