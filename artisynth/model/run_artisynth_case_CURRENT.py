#!/usr/bin/env python3
"""Run one canonical validated ArtiSynth cell with complete provenance.

The script never overwrites a existing cell. --fresh requires an absent cell
...
def create_identity(args: argparse.Namespace, cell: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    classes_dir = require_dir(args.classes_dir, "isolated validated classes directory")
    base_classes_dir = require_dir(args.base_classes_dir, "ArtiSynth models base classes directory")
    source_java = require_file(args.source_java, "validated Java source")
    ...
    if not expected_class.is_file():
        raise ValueError(f"compiled validated model class is missing: {expected_class}")
...
def verify_resume(cell_dir: Path, config: Mapping[str, Any], config_sha: str) -> tuple[str, dict[str, str]]:
    ...
    if sha256_file(config_path) != config_sha:
        raise CaseError("resume_refused", "stored configuration hash does not match current validated configuration")
    try:
    ...
    if canonical_json_bytes(stored_config) != canonical_json_bytes(config):
        raise CaseError("resume_refused", "stored configuration content does not match current validated configuration")
