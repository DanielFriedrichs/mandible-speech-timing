#!/usr/bin/env python3
"""Table-only validation of the reported ArtiSynth sensitivity outputs.

This script does not launch ArtiSynth. It independently checks the two validated
342-cell long tables, recomputes all feasibility flags from the specified
...
def main() -> int:
    ...
    endpoints.to_csv(out / "endpoints_recomputed.csv", index=False, float_format="%.12g")
    comparison.to_csv(out / "mode_comparison_recomputed.csv", index=False, float_format="%.12g")
